...
uniform vec4 fogColor = vec4(0.51, 0.51, 0.51, 1.0);
...
uniform float cloudType = STRATOCOMULUS;
uniform float time;
uniform float timeOfDay;
uniform float brightness = 1.;
uniform float cloudMoveSpeed = 10;
...
uniform float uvScaleCoverage = 1;
...
uniform float fogDensity = .5;
uniform float skyFarMultiplier = 2;

uniform int nSamples = 128;

uniform float worldRadius = 2000;
uniform float cloudStartRange = 1500;
uniform float cloudEndRange = 4000;
float sampleLength = (cloudEndRange - cloudStartRange) / float(nSamples);
// float worldRadius = 6378000;
...
vec3 applyFog(vec3 originalColor, float depth, float densityMultiplier) {

	float cd = depth * fogDensity;
	float fog = exp2(-cd * cd);

	return mix(fogColor.rgb, originalColor, saturate(fog));
}

// 0 -> stratus
// 1 -> stratocumulus
// 2 -> cumulus
float heightGradientFactor(float h, float cloudType) {
	float cumulusGradient = saturate(remap(h, 0.0, 0.1, 0.0, 1.0)) * saturate(remap(h, 0.6, 0.8, 1.0, 0.0));

	float stratusGradient = saturate(remap(h, 0.0, 0.05, 0.0, 1.0)) * saturate(remap(h, 0.1, 0.15, 1.0, 0.0));

	float stratocumulusGradient = saturate(remap(h, 0.0, 0.1, 0.0, 1.0)) * saturate(remap(h, 0.2, 0.3, 1.0, 0.0));

	if(cloudType > 1)
		return mix(stratocumulusGradient, cumulusGradient, cloudType - 1.);

	return mix(stratusGradient, stratocumulusGradient, cloudType);
}
...
float phaseFunction(float g, float costh) {
	...
	// float h = henyeyGreenstein(g, costh);
...
int raySphereIntersection(ray_t r, vec3 spherePos, float sphereRadius, out vec3 hitPoint) {
...
vec3 getRayDir() {
	vec3 ray_nds = vec3(fCoordTex.xy, 1);
	ray_nds.xy = (ray_nds.xy * 2) - vec2(1);

	vec4 ray_clip = vec4(ray_nds.xy, -1.0, 0.0);

	vec4 ray_eye = inverse(P) * ray_clip;
	ray_eye = vec4(ray_eye.xy, -1.0, 0.0);

	vec4 ray_wor = (inverse(V) * ray_eye);

	ray_wor = camRotation * ray_wor;

	return normalize(ray_wor.xyz);
}

// https://en.wikipedia.org/wiki/Spherical_coordinate_system#Unique_coordinates
vec3 getUVW(vec3 pos, vec3 domeOrigin, float tilingFactor) {

	pos -= domeOrigin;

	float r = sqrt(pos.x * pos.x + pos.y * pos.y + pos.z * pos.z);
	float polar = acos(pos.x / r);
	float azimuth = acos(pos.z / r);
	// float azimuth = sign(pos.y) * acos(pos.x / sqrt(pos.x * pos.x + pos.y * pos.y));

	r = remap(r, worldRadius + cloudStartRange, worldRadius + cloudEndRange, 0, 1);
	polar = polar / M_PI;
	azimuth = azimuth / M_PI;
	// azimuth = (azimuth + M_PI) / (2 * M_PI);

	r = saturate(r);
	polar = saturate(polar);
	azimuth = saturate(azimuth);

	return vec3(azimuth * tilingFactor, polar * tilingFactor, r);

	// vec3 d = normalize(pos - domeOrigin);
	// float u = .5 + (atan(d.z, d.x) / (2 * M_PI));
	// float v = .5 + (asin(d.y) / M_PI);
	// 	// float u = .5 + (atan(d.z, -d.y) / (2 * M_PI));
	// 	// float v = .5 + (asin(d.x) / M_PI);

	// float currentPathDistance = distance(pos, pathMarchOrigin);
	// float w = currentPathDistance / totalPathDistance;

	// return vec3(u, v, w);
}

vec3 getBoxUVW(vec3 p, vec3 minBound, vec3 maxBound, float tilingFactor) {
...
float sampleDensity(vec3 uvw, int doCheaply, float mipLevel) {
	...
	float cloud_speed = cloudMoveSpeed / (cloudEndRange - cloudStartRange);

    // cloud_top offset - push the tops of the clouds along this wind direction by this many units.  
	float cloud_top_offset = 50.0 / (cloudEndRange - cloudStartRange);  

    // // skew in wind direction  
	uvw += height_fraction * wind_direction * cloud_top_offset;  

    // //animate clouds in wind direction and add a small upward bias to the wind direction  
	uvw += (wind_direction + vec3(0.0, 0.00, 0.0)) * time * cloud_speed;
	...
	float density_height_gradient = saturate(heightGradientFactor(height_fraction, cloudType));

    // apply the height function to the base cloud shape  
	base_cloud *= density_height_gradient; //* smoothstep(0., 1., 1. - height_fraction);
	...
	cloud_coverage = pow(cloud_coverage, power);

	// cloud_coverage = saturate(cloud_coverage);
	...
	if(doCheaply != 1) {
		...
		float normalizedNoiseScale = curlNoiseScale / (cloudEndRange - cloudStartRange);
...
float sampleCloudDensityAlongCone(ray_t lightRay, float sampleLength, vec3 domeOrigin) {
	...
	for(int i = 0; i <= lightSampleCount; i++) {  
		...
		vec3 samplePos = lightRay.origin + lightRay.dir * sampleLength * float(i + 1) + getNoiseValue() * float(i + 1);
		// lightRay.origin = lightRay.origin + lightRay.dir * cone_spread_multplier + getNoiseValue() * float(i + 1);
		// vec3 samplePos = lightRay.origin;

        // sample cloud density the expensive way  
		float mip_offset = int(i * 0.5);

		uvw = getUVW(samplePos, domeOrigin, uvScaleBase);
	...
	return saturate(density_along_cone);
...
float calculateLightEnergy(ray_t lightRay, vec3 domeOrigin, float cloudDensity, float stepLength, float mu) {
	float lightRayDensity = 0.0;

	lightRayDensity = sampleCloudDensityAlongCone(lightRay, stepLength, domeOrigin);

	float beersLaw = MultipleOctaveScattering(lightRayDensity, mu);
	float powder = (1.0 - exp(-cloudDensity * 2.0 * EXTINCTION_MULT));

	return beersLaw * mix(2.0 * powder, (1.0), remap(mu, -1.0, 1.0, 0.0, 1.0)) * brightness;
}

float lightEnergy(float cosAngle, float densitySample, float originalDensity) {

	float beerLamber = exp(-densitySample * EXTINCTION_MULT);

	float powder = 1. - exp(-originalDensity * 2.);
	powder = mix(1., powder, saturate((cosAngle * .5) + .5));

	float hg = mix(henyeyGreenstein(-cosAngle, 0.8), henyeyGreenstein(-cosAngle, -0.5), 0.5);

	return 2. * beerLamber * powder * hg * brightness;
...
cloudValue_t computeCloud() {
	...
	// NOTE cambiar para alinear el cielo en "centro de tierra" en vez de en jugador
	vec3 domeOrigin = camPos - vec3(0, worldRadius, 0);

	vec3 pathMarchOrigin;
	if(raySphereIntersection(ray, domeOrigin, abs(worldRadius) + cloudStartRange, pathMarchOrigin) == 0) {
		cloudValue.transmitance = -1;
		return cloudValue;
	}

	vec3 pathMarchEnd;
	if(raySphereIntersection(ray, domeOrigin, abs(worldRadius) + cloudEndRange, pathMarchEnd) == 0) {
		cloudValue.transmitance = -1;
		return cloudValue;
	}

	float totalPathDistance = distance(pathMarchEnd, pathMarchOrigin);
	sampleLength = totalPathDistance / float(nSamples);

	float cos_angle = dot(ray.dir, -sunDir);

	cos_angle = remap(cos_angle, -1, 1, 0, 1);

	float sampleLength = totalPathDistance / float(nSamples);
	float maxSampleLength = (cloudEndRange - cloudStartRange) / float(nSamples);
	...
	float normalizedSampleLength = sampleLength / (totalPathDistance);
	...
	int zeroDensitySamples = 0;

	while(traveledDistance < totalPathDistance) {
		vec3 uvw = getUVW(ray.origin, domeOrigin, uvScaleBase);

		int doCheaply = zeroDensitySamples > 10 ? 1 : 0;
		doCheaply = 0;

		float density = sampleDensity(uvw, doCheaply, 0.);

		if(density > 0.01) {
			// backtrack to not lose info
			if(zeroDensitySamples > 10) {
				// ray.origin = ray.origin - ray.dir * sampleLength;

				// uvw = getUVW(ray.origin, domeOrigin, uvScaleBase);

				// density = sampleDensity(uvw, 0, 0.);
			}

			zeroDensitySamples = 0;
			...
			float dl = sampleCloudDensityAlongCone(lightRay, lightSampleLength, domeOrigin);

			// float lightEnergy = avg(ambient) + getLightEnergy(uvw.z, dl, density, eccentrity, cos_angle, lightSampleLength, brightness);

			float lightEnergy2 = avg(ambient) + lightEnergy(cos_angle, dl, density);

			float lightEnergy3 = avg(ambient) + calculateLightEnergy(lightRay, domeOrigin, density, lightSampleLength, cos_angle);

			float T = exp(-(density * sampleLength * EXTINCTION_MULT));

			// float integScatt = density * (lightEnergy - lightEnergy * T) / density;
			float integScatt2 = density * (lightEnergy2 - lightEnergy2 * T) / density;
			float integScatt3 = density * (lightEnergy3 - lightEnergy3 * T) / density;

			float light_intensity = integScatt3 * cloudValue.transmitance;

			cloudValue.color += vec3(lightColor * light_intensity);
		...
		if(zeroDensitySamples > 10) {
			ray.origin = ray.origin + ray.dir * sampleLength * 2;
			traveledDistance += sampleLength * 2;
		} else {
			ray.origin = ray.origin + ray.dir * sampleLength;
	...
	float dist = distance(camPos.xz, ray.origin.xz) / (far * skyFarMultiplier);

	// pasamos a rango [near, far]. lo de near es mentira pero no me importa
	dist *= 1000;

	cloudValue.color = applyFog(cloudValue.color, dist, skyFarMultiplier);

	return cloudValue;
}

vec4 getSkyColor() {
	vec2 uv = vec2(timeOfDay);

	return texture2D(skyGradient, uv);
}

void main() {
	vec4 finalColor = vec4(0);

	vec4 skyColor = vec4(0.48, 0.7, 0.98, 1.0);
	skyColor = getSkyColor();

	vec2 uvCoords = fCoordTex;

	vec4 baseColor = texture2D(textureColor, uvCoords);
	vec4 depth = texture2D(textureDepth, uvCoords);
	...
	if(depth.z == 1) {
		...
		if(cloud.transmitance < 0) {
			gl_FragColor = vec4(1, 0, 1, 1);
			return;
		}
		float t = saturate(cloud.transmitance);

		// finalColor.rgb = mix(skyColor.rgb, cloud.color, t);
		finalColor.rgb = skyColor.rgb * t + cloud.color;
	} else {
		finalColor = vec4(applyFog(finalColor.rgb, linearizeDepth(depth.z), 1.), 1);
	}
